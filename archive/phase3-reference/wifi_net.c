                               ...
                               int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        mdns_init();
        mdns_hostname_set("bosch-tank");
        mdns_instance_name_set("bosch-tank Water Tank Controller");
 ...
 * 3. Initialize mDNS (bosch-tank.local)
...
esp_err_t init_wifi(void)
{
    ...
    ESP_LOGI(TAG, "  - mDNS: bosch-tank.local (after STA connection)");
